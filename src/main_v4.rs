...
const V4_MAX_TX_POWER_DBM: i8 = 28;
const V3_MAX_TX_POWER_DBM: i8 = 22;
...
impl BatteryState {
    ...
    fn update(&mut self, adc_value: u32, solar_adc_value: Option<u32>) {
        ...
        #[cfg(feature = "v4")]
        {
            self.solar_voltage_mv = solar_adc_value.map(|value| {
                let solar_vadc_mv = (value * ADC_VREF_MV) / ADC_MAX_VALUE;
                ((solar_vadc_mv as f32) * BATTERY_DIVIDER_RATIO) as u32
            });
        }
        ...
        self.is_charging = solar_adc_value.is_some_and(|value| value > 0) || self.voltage_mv > 4200;
    }
    ...
    DIO1: embedded_hal::digital::InputPin,
{
    ...
    fn update_battery(&mut self, adc_value: u32, solar_adc_value: Option<u32>) {
        self.battery.update(adc_value, solar_adc_value);
    ...
    fn process_at_command(&mut self, uart: &UartDriver) {
        ...
        if cmd.starts_with(b"AT+VERSION") || cmd.starts_with(b"ATI") {
            ...
            if let Some(power) = parse_i8_from_cmd(&cmd[11..]) {
                if power >= -9 && power <= max_tx_power_dbm() {
                    ...
                    let _ = uart.write(b"ERROR: Power must be -9 to +28\r\n");
                }
            } else {
        ...
        let config = match protocol {
            ...
            Protocol::RNode => {
                ...
                RadioConfig {
                    ...
                    tx_power: cfg.tx_power.min(max_tx_power_dbm()),
                    sync_word: 0x12,
                    preamble_length: 8,
                    crc_enabled: true,
                    implicit_header: false,
...
fn run_lunarcore() -> ! {
    ...
    let mut solar_channel = AdcChannelDriver::new(&adc1, peripherals.pins.gpio2, &adc_config).ok();
    log::info!("[INIT] ADC OK (battery GPIO{}, solar GPIO{} optional)", PIN_BATTERY_ADC, PIN_SOLAR_ADC);
    ...
    loop {
        ...
        if now.wrapping_sub(battery_check_interval) >= 10_000 {
            ...
            if let Ok(adc_value) = adc1.read(&mut battery_channel) {
                let solar_adc_value = solar_channel.as_mut().and_then(|channel| adc1.read(channel).ok()).map(|value| value as u32);
                lunarcore.update_battery(adc_value as u32, solar_adc_value);
            }
        }
